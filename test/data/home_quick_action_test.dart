...
void main() {
  test('v20 migration adds only false flag, preserving all existing facts and metadata', () async {
    ...
    await app.database.execute('ALTER TABLE routines DROP COLUMN show_in_home_quick_actions');
    await app.database.execute('PRAGMA user_version = 20');
    final names = (await app.database.rawQuery("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name")).map((r) => r['name']! as String).toList();
    final before = {for (final name in names) name: await app.database.query(name)};
    await app.close();
    app = await AppDatabase.open(path);
    try {
      expect((await app.database.rawQuery('PRAGMA user_version')).single.values.single, 21);
      for (final name in names) {
        final rows = [for (final row in await app.database.query(name)) Map<String, Object?>.from(row)];
        if (name == 'routines') {
      ...
      expect((await app.database.rawQuery('PRAGMA integrity_check')).single.values.single, 'ok');
  ...
  test('flag persists, changes fingerprint, round-trips sync apply and rejects invalid values', () async {
    ...
    final repo = SqliteEventRepository(app);
    final service = RoutineService(repository: repo, newId: () => 'quick', now: () => DateTime(2026, 9, 5));
    await service.create('按需', null, RoutineRecurrence.daily, 0, type: RoutineType.onDemand);
    final before = await SqliteSyncSnapshotAdapter(app.database).read();
    await service.update((await repo.getRoutines()).single, '按需', null, RoutineRecurrence.daily, 0, showInHomeQuickActions: true);
    ...
    final record = after.records.singleWhere((r) => r.kind == SyncEntityKind.routine);
    expect(record.payload['showInHomeQuickActions'], 1);
    await const SqliteSyncMutationExecutor().applyDatabase(target, [SyncMutation.upsertRecord(record)]);
    expect((await SqliteEventRepository(target).getRoutines()).single.showInHomeQuickActions, isTrue);
    expect(await SqliteSyncReadiness(target).validate(), isEmpty);
    for (final fields in [
      {'show_in_home_quick_actions': 2},
      {'routine_type': 'scheduled'},
    ]) {
      await expectLater(app.database.update('routines', fields), throwsA(anything));
    }
    final oldRecord = SyncRecord(kind: record.kind, metadata: record.metadata, payload: Map.of(record.payload)..remove('showInHomeQuickActions'));
    final old = SyncSnapshot(protocolVersion: 7, schemaVersion: 20, exportedAtUtc: DateTime.utc(2026), records: [oldRecord], lists: const []);
    final upgraded = SyncSnapshot.fromJsonString(old.toJsonString());
    expect(upgraded.protocolVersion, 8);
