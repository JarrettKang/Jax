...
void main() {
  test('v15 hierarchy migrates to v16 flat standalone Events', () async {
    ...
    await old.insert('events', {
      'id': 'root', 'name': 'Root', 'status': 'paused',
      'parent_event_id': null, 'sort_order': 0, 'category_id': 'category',
      'created_at_utc': 1, 'updated_at_utc': 1,
    });
    await old.insert('events', {
      'id': 'child', 'name': 'Child', 'status': 'pending',
      'parent_event_id': 'root', 'sort_order': 0, 'category_id': null,
      'created_at_utc': 2, 'updated_at_utc': 2,
    });
    await old.close();
  ...
  test('planned source is unique and category is dynamically derived', () async {
    ...
    await db.insert('world_nodes', {
      ...
      'status': 'inProgress', 'parent_world_node_id': null, 'sort_order': 0,
      'category_id': 'category', 'created_at_utc': 1, 'updated_at_utc': 1,
    });
    await db.insert('world_nodes', {
      'id': '22222222-2222-4222-8222-222222222222', 'name': 'Child',
      'status': 'inProgress',
      'parent_world_node_id': '11111111-1111-4111-8111-111111111111',
      'sort_order': 0, 'category_id': null,
      'created_at_utc': 1, 'updated_at_utc': 1,
    });
    await db.insert('plans', {
      'id': 'plan', 'world_node_id': '22222222-2222-4222-8222-222222222222',
      'title': null, 'status': 'focused', 'round_number': 1,
      'ended_at_utc': null, 'created_at_utc': 1, 'updated_at_utc': 1,
    });
    ...
    await db.insert('events', {
      'id': 'planned', 'name': 'Planned', 'status': 'pending',
      'source_plan_item_id': 'item', 'category_id': null,
      'created_at_utc': 1, 'updated_at_utc': 1,
    });
    final repository = SqliteEventRepository(app);
    ...
    await expectLater(
      db.insert('events', {
        'id': 'duplicate', 'name': 'Duplicate', 'status': 'pending',
        'source_plan_item_id': 'item', 'category_id': null,
        'created_at_utc': 1, 'updated_at_utc': 1,
      }),
      throwsA(anything),
    ...
    await expectLater(
      db.insert('events', {
        'id': 'invalid', 'name': 'Invalid', 'status': 'pending',
        'source_plan_item_id': 'item', 'category_id': 'category',
        'created_at_utc': 1, 'updated_at_utc': 1,
      }),
      throwsA(anything),
    );
