...
void main() {
  ...
  testWidgets('drag handles reorder siblings in both directions', (
    tester,
  ) async {
    final repository = MemoryRepository([
      event('a', 0),
    ...
    Future<void> dragHandleTo(
      ...
      final sourceCenter = tester.getCenter(
        find.byKey(ValueKey('reorder-handle-$source')),
      );
      final targetBox = tester.getRect(
        find.byKey(ValueKey('reorder-target-$target')),
      );
      final targetCenter = Offset(
        targetBox.center.dx,
        after ? targetBox.bottom - 2 : targetBox.top + 2,
      );
    ...
    // First -> after last exercises sourceIndex < targetIndex adjustment.
    await dragHandleTo('a', 'd', after: true);
    expect(
      (await repository.getOrderedTopLevelEvents()).map((item) => item.id),
      ['b', 'c', 'd', 'a'],
    );

    // Last -> before first exercises sourceIndex > targetIndex.
    await dragHandleTo('a', 'b', after: false);
    expect(
      (await repository.getOrderedTopLevelEvents()).map((item) => item.id),
      ['a', 'b', 'c', 'd'],
    );
